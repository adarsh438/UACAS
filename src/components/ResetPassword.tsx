...
import { School, KeyRound, ArrowRight, AlertCircle, CheckCircle, ArrowLeft } from 'lucide-react';
...
export default function ResetPassword() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (password.length < 8 || !/\d/.test(password)) {
      setStatus('error');
      setMessage('Password must be at least 8 characters long and contain a number');
      return;
    }
      ...
      >
        <div className="flex flex-col items-center space-y-3">
          ...
          <div className="text-center space-y-1">
            <h1 className="text-2xl font-bold bg-gradient-to-r from-blue-400 to-indigo-200 bg-clip-text text-transparent tracking-tight">Set New Password</h1>
            <p className="text-slate-400 text-sm">Enter your new credentials below</p>
          </div>
        </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <input
                ...
                disabled={status === 'success' || status === 'loading' || !token}
              />
            </div>
          </div>
